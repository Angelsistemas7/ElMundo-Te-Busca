...
export function AdminDashboard({
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      ...
      {/* Solicitudes de gestor delegado: alguien logueado pide permiso sobre UN
          hospital/punto de ayuda concreto (formulario en /voluntarios/solicitar).
          Aprobar crea el ResourceManager; no otorga nada hasta que se apruebe.
          Solo admin completo — es lo mismo que Gestores dentro de cada tarjeta. */}
      {isFullAdmin && (
      <section className="mb-10">
        <h2 className="mb-3 flex items-center gap-2 font-bold text-zinc-900">
            ...
            {pendingManagerRequests.map((r) => (
              <li key={r.id} className="rounded-2xl border border-zinc-200 bg-white p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-semibold text-zinc-900">{r.username}</span>
                      <span className="text-xs text-zinc-400">quiere gestionar</span>
                      <span className="rounded-full bg-sky-50 px-2 py-0.5 text-xs font-medium text-sky-700">
                        {MANAGED_ENTITY_LABEL[r.entityType]}: {r.entityName}
                      </span>
                      <span className="text-xs text-zinc-400">{timeAgo(r.createdAt)}</span>
                    </div>
                    {r.message && <p className="mt-2 whitespace-pre-wrap text-sm text-zinc-700">{r.message}</p>}
                  </div>

                  <div className="flex shrink-0 gap-2">
                    <button
                      onClick={() => run(r.id, () => approveManagerRequestAction(r.id))}
                      disabled={pending && busy === r.id}
                      className="press flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-50"
                    >
                      <Check className="h-4 w-4" />
                    ...
                    <button
                      onClick={() => run(r.id, () => rejectManagerRequestAction(r.id))}
                      disabled={pending && busy === r.id}
                      className="press flex items-center gap-1.5 rounded-lg border border-zinc-300 px-3 py-2 text-sm font-medium text-zinc-600 hover:bg-zinc-50 disabled:opacity-50"
                    >
                      <X className="h-4 w-4" />
